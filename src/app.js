...
function clearAppConfigs( noClear ) {
    noClear = noClear.map( s => `${ s[0] }.${ s[1] }` ) ?? [];
    currentLEDs.forEach( ( controller, cIndex ) => {
        controller.forEach( ( strip, sIndex ) => {
            if ( strip.trigger === "app" && !noClear.includes( `${ cIndex }.${ sIndex }` ) ) {
                ...
                controller[sIndex] = blankStrip( strip );
            }
        } );
    } );
}
...
function writeConfigToStrips( stripIndex, options ) {
    ...
    let oldColorArr = [];
    let currentStrip = currentLEDs[stripIndex[0]][stripIndex[1]];
    if ( currentStrip.effect ) {
        clearInterval( currentStrip.effectTimout );
        currentStrip.effect = {};
    }
    if ( currentStrip.transition ) {
        clearInterval( currentStrip.transition.interval );
    }
    ...
    const stripConfiguration = controllersConfig[stripIndex[0]].strips[stripIndex[1]];
    //generate pattern
