...
module.exports = {
	...
	'generate': ( numLEDs, { colors, continues } ) => {
		...
		let colorArray = colors.map( c => new Color( c, 'hexText' ) );
		...
		for( let i = 0; i < seqLengths.length; i++ ) {
			...
			for( let j = 0; j < seqLengths[i]; j++ ) {
				arr.push( colorArray[i + offset].lerp( colorArray[i + 1 + newOffset + offset], j / ( seqLengths[i] - ( seqLengths.length - 1 === i ? 1 : 0 ) ) ).getHex() );
			}
			offset += newOffset;
		}
		return arr;
